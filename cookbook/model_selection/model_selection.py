...
# %%
model.reset_recursive()
model.run_recursive()

# %%
param = model.find_recursive("x1_effect").switch
...
# %%
a = model.find_recursive("a")
a.reset_recursive()
# a.q.loc.loader.value.data[:] = 0.
# a.q.scale.loader.value.data[:] = 1.
...
# %%
model = models["linear(x1) + linear(x2)"]
model.reset_recursive()
model.run_recursive()

# %%
a = model.find_recursive("x1_effect").a
(a.likelihood - a.posterior).sum()
