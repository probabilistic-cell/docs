...
z = la.links.scalar.Spline(
    ...
    output = output_definition,
    n_knots = 20,
    step_distribution = la.distributions.Normal(scale = 3.),
    b = True
...
# %%
for kernel_id in ["gaussian", "laplace", "cauchy", "multiquadratic", "wendland_c2", "wendland_c4", "wendland_c6"]:
    kernel = getattr(la.links.scalar.spline, kernel_id)
    
    y = kernel(torch.tensor(x)).numpy()
    y = y/y.sum()
    sns.lineplot(x = x, y = y, label = kernel_id)
