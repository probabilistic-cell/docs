...
# %%
n_genes = 100
genes = la.Dim([str(i) for i in range(n_genes)], name=e="gene")
...
b = la.Parameter(
    0.0, definition=intercept, transforms=la.distributions.Normal(scale=1.0).transform_to()
)
s = la.Parameter(
...
# %%
x = la.Latent(
    la.distributions.Uniform(0.0, 3.0), definition=la.Definition([cells]), label="x"
)
