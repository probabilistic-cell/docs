...
# %%
dist1.log_prob(torch.tensor([0., 0., 0., 0., 0.]))
...
sns.scatterplot(
    x = angle.prior_pd(),
    y = la.domains.circular.mean(observed.samples[a], "sample").to_pandas()
)
...
causal.plot_features(observation.p);


# %%
causal.plot_features(feature_ids=genes.coords[:5]);


# %%
observed.samples[z.a].mean("sample").sel(gene = genes.coords[0])
...
sns.scatterplot(
    x = angle.prior_pd(),
    y = posterior.samples[a].mean("sample").to_pandas()
)
